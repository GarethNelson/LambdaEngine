...
int main(int argc, char** argv) {
    ...
    fprintf(visual_fd,"# Autogenerated by the lambdacut tool, do not modify by hand\n");
    fprintf(visual_fd,"VISUALMAP\n");
    fprintf(visual_fd,"WIDTH: %d\n",image_w);
    fprintf(visual_fd,"HEIGHT: %d\n",image_h);
    fprintf(visual_fd,"TILES_W: %d\n",tiles_x);
    fprintf(visual_fd,"TILES_H: %d\n",tiles_y);
    fprintf(visual_fd,"%%%%\n");
    for(i=0; i<tile_count; i++) {
        fprintf(visual_fd,"TILE\n");
        fprintf(visual_fd,"ID: %d\n",i);
        fprintf(visual_fd,"GRID_X: %d\n",(i % tiles_x));
        fprintf(visual_fd,"GRID_Y: %d\n",(i / tiles_x));
        fprintf(visual_fd,"RENDER_X: %d\n",(i % tiles_x)*tile_w);
        fprintf(visual_fd,"RENDER_Y: %d\n",(i % tiles_y)*tile_h);
        if(is_dup[i]==-1) {
